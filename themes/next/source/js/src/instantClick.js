...
var InstantClick = function(document, location) {
  ...
  function changePage(title, body, newUrl, scrollY) {
    ...
    */
    ...
    if ($isChromeForIOS && document.title == title) {
       ...
       *
       * 2. Will not set the title if it鈥檚 identical when trimmed, so
    ...
    // Real event, useful for combining userscripts, but only for that so it鈥檚 undocumented.
  ...
  function mousedown(e) {
    if ($lastTouchTimestamp > (+new Date - 500)) {
      return // Otherwise, click doesn鈥檛 fire
    }
  ...
  function display(url) {
    ...
    if ($preloadTimer || !$isPreloading) {
      /* $preloadTimer:
         Happens when there鈥檚 a delay before preloading and that delay
         hasn't expired (preloading didn't kick in).

         !$isPreloading:
         A link has been clicked, and preloading hasn鈥檛 been initiated.
         It happens with touch devices when a user taps *near* the link,
         Safari/Chrome will trigger mousedown, mouseover, click (and others),
         but when that happens we ignore mousedown/mouseover (otherwise click
         doesn鈥檛 fire). Maybe there鈥檚 a way to make the click event fire, but
         that鈥檚 not worth it as mousedown/over happen just 1ms before click
         in this situation.
    ...
    if ($isWaitingForCompletion) {
      /* The user clicked on a link while a page was preloading. Either on
         ...
         might have gone wrong (or he could have double clicked, we don鈥檛
  ...
  var bar = function() {
    ...
    function updatePositionAndScale() {
      /* Adapted from code by Sam Stephenson and Mislav Marohni膰
